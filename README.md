...
- a safe texture-aware right-side preview with **Auto (reliable)**, **3D model (experimental)**, and **2D icon / texture** modes; Auto prefers static geometry only when its material binding is trustworthy and falls back to a truthful 2D asset for ambiguous legacy renderers, while JSON textures, UV-aware OBJ textures, full two-block doors, animation-frame handling, and recognized legacy shapes remain available without executing mod code,
...
For legacy Forge mods, class/enum evidence is stronger than blindly treating every file under `textures/blocks` as registered. Backport providers can also associate a registered provider block with faithful textures intentionally packaged under `assets/minecraft`, as Et Futurum does. Dedicated legacy model textures under paths such as `textures/models/...` are linked to their OBJ/DAE geometry before ordinary block icons. Same-named legacy model families are disambiguated using block-entity package context, and model texture binding deliberately rejects unrelated same-name inventory assets (for example a machine and weapon sharing a name). For modern mods, blockstates are strong asset evidence. Block/tile entities are shown separately and do not automatically become conversion mappings. Preview mode is strictly presentation-only: it never changes the catalog mapping profile or conversion target. The preview remains a static inspection tool: packaged planar faces and OBJ UVs are texture-projected directly, while runtime-only or material-ambiguous TESRs/BERs can fall back to a linked 2D item icon/block/model texture or an explicit unresolved-runtime message instead of invented/broken geometry.
